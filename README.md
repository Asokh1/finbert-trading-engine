...
Backtests can always be written off as curve-fit in hindsight. `live_forward_test.py` runs the same sentiment-momentum signal as `backtest.py`, but on live data as of today, and it logs a prediction (entry price, ATR-based stop/target, position size, a real timestamp) before the outcome is known, into `data/forward_test.db`. Every run also grades whatever's still open against fresh price data and closes anything that's hit its stop, hit its target, or aged past the holding-period cap. What comes out is win rate, cumulative return, and Sharpe/Sortino/drawdown computed purely off that closed-prediction log, a track record that actually grows over time instead of a single backtest snapshot.

It's meant to run once a day off Task Scheduler or cron. A symbol with an open prediction gets skipped instead of re-signaled, and closed predictions never reopen, so running it twice in a row doesn't double anything up. `data/forward_test.db` is committed to the repo instead of gitignored, and gets updated as real history piles up, so the commit history itself ends up being part of the evidence.

## Alpha attribution: is sentiment adding anything?

Is the sentiment signal actually doing anything, or is this just price momentum wearing a costume? `ablation_study.py` runs the same trade-simulation machinery (ATR stops, position sizing, cost-adjusted returns, daily mark-to-market accounting) across three signal sources on the same tickers and window:
...
3. a placebo: the real sentiment scores, with the article-to-date mapping shuffled (fixed seed) before folding into the momentum calc, which kills any genuine predictive relationship while keeping the score distribution and roughly the same trade frequency

Price returns and shuffled sentiment scores sit on a totally different numeric scale than the sentiment MACD, so each control arm gets its own threshold: a quantile cutoff against that arm's own realized momentum distribution over the test window, picked to match the sentiment arm's empirical trade rate rather than reusing one literal cutoff across arms. One caveat here: calibrating off the full-window distribution is itself a mild form of look-ahead, since it uses the whole test period to decide where the line gets drawn. It only affects the two control arms' thresholds though, not the sentiment arm's original hand-tuned threshold and not any individual trade's entry/exit logic, but it's still a simplification worth knowing is there.
...
Two things affect how you should read that backtest result above. First, the ~355-day window is close to the limit of what Finnhub's free tier actually returns for news; go back further than roughly 365 days and requests start coming back empty. Second, an earlier version of this backtest had a real bug: Finnhub read-timeouts were caught with a bare `except: pass`, so once the free tier started throttling repeated requests, a date that failed to fetch news looked identical in the output to a date with no news at all. No error, nothing. That was quietly dropping about two-thirds of the candidate dates. Fixing it (`api_utils.py`, longer timeout, retries with backoff, a floor delay between requests) roughly tripled the trade count and moved the Sharpe/Sortino numbers up noticeably, which is why they won't match figures from earlier commits. That older sample was incomplete and gave no indication of it.

The momentum threshold (`|momentum| > 0.01`) is tuned on the same window it's tested on rather than a separate calibration split, and a rough holdout check showed the typical size of the momentum signal isn't stable over time, so a threshold tuned on one period can misfire on another. That's an open problem here, still unsolved. Over this window the strategy trailed SPY buy-and-hold, mostly because SPY had an unusually strong run and a long/short strategy taking 43 trades across 5 names was never going to capture that much of a broad market move. This project isn't trying to prove profitability; the point is the risk and evaluation machinery around it — ATR-based stops and sizing, cost-adjusted PnL, daily mark-to-market accounting for overlapping trades, and reporting results against a real benchmark instead of in isolation.
